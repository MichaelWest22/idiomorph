describe("Core morphing tests", function () {
  ...
  it("morphs innerHTML using meta config defined inner config", function () {
    // inner config set in web-test-runner.config.mjs via a meta tag
    let initial = make("<div>Foo</div>");
    let finalSrc = "<button>Bar</button>";
    let final = make(finalSrc);
  ...
  it("morphs innerHTML using addConfig", function () {
    ...
    let finalSrc = "<button>Bar</button>";
    let final = make(finalSrc);
    Idiomorph.addConfig("inner2", { morphStyle: "innerHTML" });
    Idiomorph.morph(initial, final, "inner2");
    initial.outerHTML.should.equal("<div><button>Bar</button></div>");
  });
